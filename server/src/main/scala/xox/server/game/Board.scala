...
sealed abstract class SetResult extends BoardLike {
  override def get(x: Int, y: Int): Option[Field] =
    Some(this).collect { case SetResult.Ok(updatedBoard) => updatedBoard.get(x, y) }
     .flatten
...
final case class Board private(size: Int,
                               private val fields: List[Field],
                               private val free: Int) extends BoardLike {
  override def get(x: Int, y: Int): Option[Field] =
    Some(x -> y)
  ...
  private def winInMainDiagonal(x: Int, y: Int, mark: Mark): Boolean = {
    ...
    0.until(size).map(i => index(i, i)).filterNot(_ == fieldIndex)
      .map(fields(_))
      .forall(_ == Field.Taken(mark))
  }
  ...
  private def winInSideDiagonal(x: Int, y: Int, mark: Mark): Boolean = {
    val fieldIndex = index(x, y)
    0.until(size).map(i => index(size - i - 1, i)).filterNot(_ == fieldIndex)
      .map(fields(_))
      .forall(_ == Field.Taken(mark))
  }
