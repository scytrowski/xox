...
trait ServerState {
  ...
  def createMatch(ownerId: String, parameters: MatchParameters): CreateMatchResult
...
object ServerState {
  ...
  object LoginResult {
    final case class Ok(state: ServerState, playerId: String) extends LoginResult
  ...
  object CreateMatchResult {
    final case class Ok(state: ServerState, matchId: String) extends CreateMatchResult
  ...
  object JoinMatchResult {
    final case class Ok(state: ServerState, ownerId: String, ownerMark: Mark) extends JoinMatchResult
...
final case class ServerStateLive(players: Map[String, Player],
                                 matches: Map[String, Match],
                                 private val idGenerator: IdGenerator,
                                 private val matchStateFactory: MatchStateFactory) extends ServerState {
  def login(nick: String, clientId: String): LoginResult =
  ...
  def joinMatch(matchId: String, playerId: String): JoinMatchResult =
    (findPlayerById(playerId), findMatchById(matchId), findMatchByPlayerId(playerId)) match {
      case (Some(_), Some(notStarted: Match.WaitingForOpponent), None) =>
        ...
        val updatedState = copy(matches = matches.updated(startedMatch.id, startedMatch))
        JoinMatchResult.Ok(updatedState, startedMatch.ownerId, startedMatch.state.ownerMark)
