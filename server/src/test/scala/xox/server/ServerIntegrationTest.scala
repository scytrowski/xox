...
class ServerIntegrationTest
    extends StreamSpec("ServerIntegrationTest")
    with ServerFixture
    with ConfigFixture
    with ClientFixture
    with TryValues
    ...
    "handle chunk of commands" in {
      ...
      withServer(
        ...
        withClient(binding.localAddress) { client =>
          ...
          expectLoginResults(client, "abc", "def", "ghi") must contain theSameElementsInOrderAs List(
            firstPlayerId,
            secondPlayerId,
            thirdPlayerId
          )
    ...
    "handle concurrent clients" in {
      ...
      withServer(appConfig, clientIds ++ playerIds: _*) { binding =>
        withClients(binding.localAddress, clientsCount) { clients =>
          ...
          clients.zip(nicks).foreach {
            case (client, nick) => client.send(Login(nick))
          }
  ...
  private def expectLoginResults(
      client: TestClient,
      nicks: String*
  ): List[String] =
    client
      .receiveN(2 * nicks.length)
      .grouped(2)
      .toList
      .zip(nicks)
      .map {
        case (commands, nick) =>
          inside(commands) {
            case LoginOk(_) :: PlayerLogged(playerId, n) :: Nil if n == nick =>
              playerId
          }
      }

  private def expectCreateMatchResult(
      client: TestClient,
      ownerId: String,
      parameters: MatchParameters
  ): String =
    inside(client.receiveN(2)) {
      case CreateMatchOk(_, oId1) :: MatchCreated(matchId, oId2, p) :: Nil
          if oId1 == ownerId && oId2 == ownerId && p == parameters =>
        matchId
    }

  private def expectJoinMatchResult(
      client: TestClient,
      matchId: String,
      opponentId: String
  ): Mark =
  ...
  private def randomStrings(count: Int, length: Int = 10) =
    List.fill(count)(randomString(length))

  private def randomString(length: Int = 10) =
    List.fill(length)(Random.nextPrintableChar()).mkString
}
